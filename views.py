            ...
            with open(photo, 'rb') as im:
                ...
                for label in new_labels:
                    if  label['Name'] == 'Firearm'or label['Name'] == 'Weapon' or label['Name'] == 'Gun' or \
                        ...
                        to_mail = ["jeffijapheth428@gmail.com"]
                        print('success')
                        try:
...
def user_video_rekognition(request):
    ...
    if request.method == 'POST':
        ...
        for label in new_labels:
            if label["Label"]['Name'] == 'Firearm' or label["Label"]['Name'] == 'Weapon' or \
                label["Label"]['Name'] == 'Gun' or label["Label"]['Name'] == 'Handgun' or \
                label["Label"]['Name'] == 'Grenade' or label["Label"]['Name'] == 'Ammunition' or \
                label["Label"]['Name'] == 'Blade' or label["Label"]['Name'] == 'Dagger':
                weapon=True
        ...
        if weapon == False:
            ...
            dem['Label'] = label
            new_labels.append(dem)
            messages.info(request, 'Weapon Not Found')
        print('----')
        print(new_labels)
        # sorted = sorted(labels, key=itemgetter('name'), reverse=True)
        # print(type(labels))
        # sorted_footballers_by_goals = sorted(labels.items(), key=lambda x:x[1])
        # converted_dict = dict(sorted_footballers_by_goals)
        # print(labels)
        # for label in labels:
        #     if label['Label']['Name'] == 'Accessories':
        #         print(label)
        if weapon == True:
                frequency = 2500  # Set Frequency To 2500 Hertz
                duration = 1500  # Set Duration To 1000 ms == 1 second
